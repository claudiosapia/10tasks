...
//expect number 1 to be <= max value
//expect number 1 to be <= max value

  expect(Number(numb1.value)).toBeLessThanOrEqual(max);
  expect(Number(numb2.value)).toBeLessThanOrEqual(max);

  expect(Number(numb1.value)).toBeGreaterThanOrEqual(min);
  expect(Number(numb2.value)).toBeGreaterThanOrEqual(min);

  
   expect(Number(numb1.value)).toMatchSnapshot();

   expect(Number(numb2.value)).toMatchSnapshot();
});
...
test("If the user entered value matches the generated total and display feedback, Test if +1 is added to let answer variable", () => {
  userInput.value = sum;
  //assign random numbers to inputs
...
test("test if wrong answer is given correct message is displayed to the user", () => {
  answer;
  ...
  expect(showAnswer.innerHTML).toBe(
    "<h2> :(</h2> <h2>Oh No!  The Correct Answer is :</h2>" +
     ...
     expect(showAnswer.innerHTML).toMatchSnapshot();
...
test("if user answer correctly 8 times, END the quiz and display user feedback", () => {
  ...
  expect(correctAnswer.innerHTML).toBe(
   ...
   expect(correctAnswer.innerHTML).toMatchSnapshot();
});
