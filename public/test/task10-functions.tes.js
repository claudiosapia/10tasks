...
describe('TEST TASK 10.JS FUNCTIONS *CASE: USER ADDS £5 CREDIT)', () => {
...
//expect score to be £ 1.00
expect(score).toBe("£3.00")
//expect credit toBe £5
expect(credit).toBe("£5")
//alert is called times:
expect(alert).toHaveBeenCalledTimes(1);
...
expect(credit).toBe('£5 in your Wallet')
...
expect(roundN).toBe("1");
//expect score to be :
expect(score).toBe("£7.00")
//expect credit toBe :
expect(credit).toBe("£5")

expect(square1).toBe('red')
expect(square2).toBe('red')
expect(square3).toBe('red')
...
//EXPECT RESULT TO BE:
expect(result.innerHTML).toBe("You have withdrawn <span class=\"lead text-danger gameBalance\">£7.00</span> from your total game balance , You have: <p id=\"wallet\" class=\"lead text-danger\">£12 in your wallet</p>")

//EXPECT SQUARE HTML TO BE:...
expect(square1).toBe('red')  //WHITE BG COLOUR
expect(square2).toBe('red') //WHITE BG COLOUR
expect(square3).toBe('red')  //WHITE BG COLOUR
});
