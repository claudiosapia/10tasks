...
let groupA = document.getElementById("grpA");
//get fastestRunner span
let fastestRunner = document.getElementById("fastestRunner");
//get span time
let time = document.getElementById("time");
//get table runnersProgressing
let runnersProgressing = document.getElementById("runners-progressing");
...
//function generating random values of runners lap times
function randomTimes(min) {
  max = 80;
  min = 35;
...
for (let i = 0; i < runners.length; i++) {
  // create a new row
  let newRow = groupA.insertRow(groupA.length);
...
let filteredRunners = runners.filter(function (runner) {
  return runner[2] < breakpoint || runner[3] < breakpoint;
});

//display fastest runner and lap time
for (i = 0; i < filteredRunners.length; i++) {
  fastestRunner.innerHTML = filteredRunners[0][0] + " " + filteredRunners[0][1];
  //display filtered runners
  time.innerHTML = filteredRunners[0][2];

  // create a new row
  let newRow = runnersProgressing.insertRow(runnersProgressing.length);
